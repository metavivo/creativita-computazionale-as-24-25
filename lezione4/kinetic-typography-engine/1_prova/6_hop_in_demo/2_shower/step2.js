...
function setup() {
  ...
  for (let i = 0; i < numDrops; i++) {
    ...
    let y = -random(100, 300); // Le gocce partono dall'alto
    let r = 30 * (random() - 0.5);
    drops.push({
...
function draw() {
  ...
  for (let drop of drops) {
    ...
    if (dropElapsed >= 0 && dropElapsed < drop.duration) {
      ...
      let dropFall = easeOutExpo(t) * 200; // Movimento verticale verso il basso
      
      push();
      translate(width / 2 + drop.xOffset, height / 2 + drop.yOffset + dropFall);
