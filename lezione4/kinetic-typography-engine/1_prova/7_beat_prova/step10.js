...
let currentTextSize = 44; // Inizializzato alla dimensione attuale
...
let phraseInput; // Area di testo per personalizzare la frase
...
function setup() {
    ...
    phraseInput = createElement('textarea');
    phraseInput.position(20, height + 20); // Subito sotto il canvas
    phraseInput.size(width - 40, height / 4); // Metà dell'altezza precedente
    phraseInput.value("Nel mezzo del cammin di nostra vita mi ritrovai per una selva oscura, ché la diritta via era smarrita. Ahi quanto a dir qual era è cosa dura esta selva selvaggia e aspra e forte che nel pensier rinova la paura! Tant’è amara che poco è più morte");

    songTimeLabel = createDiv("Tempo: 0:00");
    songTimeLabel.position(20, height + 140 + height / 4); // Spostata sotto l'area di testo

    beatThresholdSlider = createSlider(0, 1, beatThreshold, 0.01);
    beatThresholdSlider.position(20, height + 160 + height / 4);
    beatThresholdSlider.style('width', '300px');
    thresholdValueLabel = createDiv("Soglia Beat: " + beatThreshold.toFixed(2));
    thresholdValueLabel.position(330, height + 160 + height / 4);
    ...
    startTimeSlider.position(20, height + 200 + height / 4);
    ...
    startTimeValueLabel.position(330, height + 200 + height / 4);
    currentStartTime = 10;

    beatCooldownSlider = createSlider(0, 500, beatCooldown, 10);
    beatCooldownSlider.position(20, height + 240 + height / 4);
    beatCooldownSlider.style('width', '300px');
    beatCooldownValueLabel = createDiv("Cooldown Beat: " + beatCooldown);
    beatCooldownValueLabel.position(330, height + 240 + height / 4);
    ...
    lowPassFreqSlider.position(20, height + 280 + height / 4);
    lowPassFreqSlider.style('width', '300px');
    lowPassFreqValueLabel = createDiv("Freq. Low Pass: " + 970);
    lowPassFreqValueLabel.position(330, height + 280 + height / 4);
    ...
    lowPassResSlider.position(20, height + 320 + height / 4);
    lowPassResSlider.style('width', '300px');
    lowPassResValueLabel = createDiv("Res. Low Pass: " + 1.0);
    lowPassResValueLabel.position(330, height + 320 + height / 4);
    ...
    textSizeSlider.position(20, height + 440 + height / 4); // Posizionato sotto l'ultimo slider
    textSizeSlider.style('width', '300px');
    textSizeValueLabel = createDiv("Dimensione Testo 2: " + currentTextSize);
    textSizeValueLabel.position(330, height + 440 + height / 4);
    ...
    hopCheckbox.position(20, height + 480 + height / 4);

    kineticCheckbox = createCheckbox('Mostra Tipografia Cinetica', showKineticAnimation);
    kineticCheckbox.position(200, height + 480 + height / 4); // Spostata un po' a destra
    ...
    kineticCheckbox.changed(() => {
        ...
        if (showKineticAnimation) {
            phrase = phraseInput.value().split(" ");
        }
    });
...
function draw() {
    ...
    fill(255, 100);
    textSize(12);
    text("Soglia Beat: " + beatThreshold.toFixed(2), 20, height + 20 + height / 4 + 20);
    text("Cooldown Beat: " + beatCooldown, 20, height + 60 + height / 4 + 20);
    text("Freq. Low Pass: " + lowPass.freq().toFixed(1), 20, height + 100 + height / 4 + 20);
    text("Res. Low Pass: " + lowPass.res().toFixed(1), 20, height + 140 + height / 4 + 20);
    text("Partenza al secondo: " + currentStartTime, 20, height + 180 + height / 4 + 20);
}
